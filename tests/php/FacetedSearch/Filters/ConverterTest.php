...
class ConverterTest extends MockeryTestCase
{
    ...
    /**
     * Fork Batinea: a feature value can carry a swatch, fed by the creafeatures module — an image
     * in img/fv/{id}.jpg, or a colour code in feature_value.color. The image wins when both exist,
     * mirroring what the core does for attribute colours.
     */
    public function testGetFacetsFromFilterBlocksExposesFeatureValueSwatches()
    {
        $facets = $this->converter->getFacetsFromFilterBlocks([
            [
                'type_lite' => 'id_feature',
                ...
                'meta_title' => null,
                'values' => [
                    // An image exists for this one (tests/php/files/fv/12.jpg), and it takes
                    // precedence over the colour code.
                    12 => ['nbr' => '1', 'name' => 'Brushed', 'url_name' => null, 'meta_title' => null, 'color' => '#c8a165'],
                    13 => ['nbr' => '1', 'name' => 'Gold', 'url_name' => null, 'meta_title' => null, 'color' => '#c8a165'],
                    14 => ['nbr' => '1', 'name' => 'Plain', 'url_name' => null, 'meta_title' => null],
                ],
                'filter_show_limit' => '0',
                'filter_type' => '0',
            ],
        ...
        $filters = [];
        foreach ($facets[0]->getFilters() as $filter) {
            $filters[$filter->getLabel()] = $filter;
        }

        $this->assertSame('/img/fv/12.jpg', $filters['Brushed']->getProperty('texture'));
        $this->assertNull($filters['Brushed']->getProperty('color'));

        $this->assertSame('#c8a165', $filters['Gold']->getProperty('color'));
        $this->assertNull($filters['Gold']->getProperty('texture'));

        $this->assertNull($filters['Plain']->getProperty('color'));
        $this->assertNull($filters['Plain']->getProperty('texture'));
    }

    /**
     * Attribute colours must keep resolving against img/co/, untouched by the feature value branch.
     */
    public function testGetFacetsFromFilterBlocksKeepsAttributeColorsOnTheirOwnDirectory()
    {
        $facets = $this->converter->getFacetsFromFilterBlocks([
            [
                ...
                'meta_title' => null,
                'values' => [
                    12 => ['nbr' => '1', 'name' => 'Taupe', 'url_name' => null, 'meta_title' => null, 'color' => '#CFC4A6'],
                ],
                'filter_show_limit' => '0',
                'filter_type' => '0',
            ],
        ...
        $this->assertSame('/theme/12.jpg', $filters[0]->getProperty('texture'));
    }
