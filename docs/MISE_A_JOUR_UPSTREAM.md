...
C'est le point le plus important : **seuls 4 fichiers** divergent. Tant que cette liste reste juste, le rebase est indolore.

| Fichier | Nature de l'écart |
| --- | --- |
| `src/CombinationFeature.php` | `isFilteringEnabled()` renvoie `true` en dur (upstream : PS >= 9.3 + feature flag `combination_feature_values`) |
| `src/Product/SearchProvider.php` | ajout de `injectFeatureMatchedCombinations()` + son appel dans `runQuery()` |
...
| `docs/` | fichier ajoutés par le fork |
